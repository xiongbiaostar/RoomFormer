...
def prepare_for_dn(dn_args, tgt_weight, embedweight, batch_size, training, num_queries, num_classes, hidden_dim, label_enc,num_query_per_poly=40,scene_ids=None):
    ...
    #tgt_weight[800,256] label_enc:embedding[2,255]
    if tgt_weight is not None and embedweight is not None:
        # decoder的content query和position query
        # content query
        indicator0 = torch.zeros([num_queries, 1]).cuda()#[800,1]
        # sometimes the target is empty, add a zero part of label_enc to avoid unused parameters
        tgt = torch.cat([tgt_weight, indicator0], dim=1) + label_enc.weight[0][0] * torch.tensor(0).cuda()#[800,256]+[800,1]=[800,257]
        # position query
    ...
    if training:
        ...
        known = [(torch.ones_like(t['labels'])).cuda() for t in targets] #一个batch的标签列表
        ...
        known_num = [sum(k) for k in known]#一个batch中，每个场景的边数
        ...
        labels = torch.cat([t['labels'] for t in targets]) #有效点/无效点[edge_num_allbatch]
        ...
        # prob_point = 0.5
        modify_flag = False  # torch.rand(1).item() < prob_point #有50%的概率会进行顶点增减
        #
        ...
        input_label_embed = label_enc(m) #[2035,256]
        # add dn part indicator
        indicator1 = torch.ones([input_label_embed.shape[0], 1]).cuda() #[2035,1]
        ...
        padding_coords = torch.zeros(pad_size, 4).cuda() #每个坐标两个点
        ...
        #print("dnnnnnnnnnn",attn_mask.shape,tgt_size)
