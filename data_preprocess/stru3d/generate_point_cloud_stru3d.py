...
def config():
    ...
    a.add_argument('--data_root', default='/media/dv/satadisk/TransformerSegmentation_workspace/dataset/Stu3dAll_raw_data/00', type=str, help='path to raw Structured3D_panorama folder')
    args = a.parse_args()
    return args
...
def main(args):
    ...
    for part in tqdm(data_parts):
        scenes = os.listdir(os.path.join(data_root,  'Structured3D'))
        for scene in tqdm(scenes):
            ...
            save_path = os.path.join(data_root,  'Structured3D', scene, 'point_cloud.ply')
